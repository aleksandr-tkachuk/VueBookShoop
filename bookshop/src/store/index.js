...
const store = new Vuex.Store({
  ...
  actions: {
    async booklist ({commit}) {
      try {
        let {data: {data: res}} = await axios.get('http://bookshop/api/getBooks')
    ...
    async genres ({commit}) {
      try {
        let {data: {data: res}} = await axios.get('http://bookshop/api/getGenres')
    ...
    async authors ({commit}) {
      try {
        let {data: {data: res}} = await axios.get('http://bookshop/api/getAuthors')
...
//      let user = null
      if (_.isObject(creds)) {
        try {
          let response = await axios.post('http://bookshop/api/auth', creds)
...
//          commit('set', {type: 'user', items: user})
    ...
    async getuser ({commit}, token) {
      ...
      if (token) {
        try {
          let response = await axios.get('http://bookshop/api/user', {headers: {'token': token}})
          user = _.get(response, 'data.data')
        } catch (e) {
          user = null
        }
